...
from .models import Message,Friend,Group,Good,Menber,Category
...
def post(request):
    ...
    if request.method == 'POST':
        ...
        menber = Menber.objects.get(id=request.POST['menber'])
...
def analysis(request):
    ...
    params = {
        ...
        'login_user':request.user,
        'values':['A','B','C'],
        'menbers':Menber.objects.all(),
        'categorys':Category.objects.annotate(avg_review=Avg('posts__star')),
        'CATEGORY_CHOICES':['TV','ラジオ','雑誌','ブログ','モバメ'] ,
        
    }
    return render(request, 'sns/analysis.html', params)
...
def explain(request):
    ...
    return render(request, 'sns/explain.html', params)
